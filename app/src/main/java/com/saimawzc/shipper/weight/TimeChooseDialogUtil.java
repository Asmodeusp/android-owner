...
public class TimeChooseDialogUtil {
    ...
    public void showDialogSeconds(final TimeChooseListener listener){
        //控制时间范围(如果不设置范围，则使用默认时间1900-2100年，此段代码可注释)
        //因为系统Calendar的月份是从0-11的,所以如果是调用Calendar的set方法来设置时间,月份的范围也要是从0-11
        Calendar selectedDate = Calendar.getInstance();
        Calendar startDate = Calendar.getInstance();
        ...
        pvTime = new TimePickerView.Builder(context, new TimePickerView.OnTimeSelectListener() {
            ...
            public void onTimeSelect(Date date, View v) {//选中事件回调
                // 这里回调过来的v,就是show()方法里面所添加的 View 参数，如果show的时候没有添加参数，v则为null
                listener.getTime(getTime(date,"yyyy-MM-dd HH:mm:ss"));
            }
        })
                //年月日时分秒 的显示与否，不设置则默认全部显示
    ...
    public String getTime(Date date,String formart) {//可根据需要自行截取数据显示
